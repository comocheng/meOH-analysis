...
###############################################
    ...
    def run_simulation_transient(self):
        """
        Run single reactor to a set time
        """
        gas_ROP_str = [i + " ROP [kmol/m^3 s]" for i in self.gas.species_names]
    ...
    def brute_force_sens(self):
        # run the simulation first
        output_original = self.run_simulation_transient()
        # change the enthalpy
        test_sp_id = self.surf.species_index('CH3O*(21)')
        h_original = self.surf.species(test_sp_id).thermo.h(self.temp)
        h_perturbed = h_original * 0.01
        test_sp = self.surf.species(test_sp_id)
        perturbed_coeffs = test_sp.thermo.coeffs.copy()
        perturbed_coeffs[6] += h_perturbed
        perturbed_coeffs[13] += h_perturbed
        test_sp.thermo = ct.NasaPoly2(test_sp.thermo.min_temp, test_sp.thermo.max_temp,
                                      test_sp.thermo.reference_pressure, perturbed_coeffs)
        self.surf.modify_species(test_sp_id, test_sp)
        perturbed_yaml_path = os.path.join(os.path.split(self.yaml_file)[0], 'test.yaml')
        self.surf.write_yaml(perturbed_yaml_path)
        # add element section into the new converted file (a cantera bug)
        with open(self.yaml_file, 'r') as f:
            model = yaml.load(f, Loader=yaml.FullLoader)
        with open(perturbed_yaml_path, 'r') as f:
            model_to_change = yaml.load(f, Loader=yaml.FullLoader)
        model_to_change['element'] = model['element']
        with open(perturbed_yaml_path, 'w') as f:
            yaml.dump(model_to_change, f, default_flow_style=False)
        new_simulation = sbr(perturbed_yaml_path, self.rmg_model_path, sensitivity=False)
        output_perturbed = new_simulation.run_simulation_transient()
        for i, name in enumerate(output_original[0]):
            if name  == 'CH3O*(21)':
                sp_original_idx = i
        for i, name in enumerate(output_perturbed[0]):
            if name == 'CH3O*(21)':
                sp_perturbed_idx = i
        perturbed_conc = output_perturbed[-1][sp_perturbed_idx]
        original_conc =  output_original[-1][sp_original_idx]
        bf_sens = (perturbed_conc - original_conc) / original_conc / 0.01
        return bf_sens
...
def run_sbr_test():
    ...
    """
    rmg_model_folder = "/home/chao/meOH/meOH-synthesis/"
    cti_file_path = rmg_model_folder + "base/cantera/chem_annotated.yaml"
    ...
    # run to SS
    sensitivity = sbr_ss.brute_force_sens()
    print(sensitivity)
